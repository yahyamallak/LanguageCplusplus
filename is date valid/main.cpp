...
bool isValidDate(Date date) {
    ...
    short maxDays = NumberOfDaysInAMonth(date.month, date.year);

    if(date.day <= 0 || date.day > maxDays) {
        return false;
...
int main() { 
    ...
    if(isValidDate(date)) {
        cout << "Yes, it is a valid date." << endl;
    } else {
        cout << "No, it is not a valid date." << endl;
    }
