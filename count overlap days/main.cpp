...
bool isPeriodsOverlap(Period period1, Period period2) {
    if(isDate1AfterDate2(period2.startDate, period1.endDate) || isDate1AfterDate2(period1.startDate, period2.endDate)) {
        return false;
    }
    return true;
}
...
int periodLength(Period period, bool inc = false) {
    return diffDates(period.startDate, period.endDate, inc);
}

short countOverlapDays(Period period1, Period period2) {
    if(isPeriodsOverlap(period1, period2)) {
        if(isDate1AfterDate2(period1.endDate, period2.startDate)) {
            return diffDates(period2.startDate, period1.endDate);
        } else if(isDate1AfterDate2(period2.endDate, period1.startDate)) {
            return diffDates(period1.startDate, period2.endDate);
        }
    }
    return 0;
}
...
int main() { 
    ...
    period1.startDate = enterDate(period1.startDate);

    cout << "Enter end date : " << endl;

    period1.endDate = enterDate(period1.endDate);
    ...
    period2.startDate = enterDate(period2.startDate);

    cout << "Enter end date : " << endl;

    period2.endDate = enterDate(period2.endDate);

    cout << "Count overlap days : " << countOverlapDays(period1, period2) << endl;
