...
bool isDateWithinPeriod(Period period, Date date) {
    if(isDate1AfterDate2(date, period.endDate) || isDate1AfterDate2(period.startDate, date)) {
        return false;
    }
    return true;
}
...
int main() { 
    ...
    period.startDate = enterDate(period.startDate);

    cout << "Enter end date : " << endl;

    period.endDate = enterDate(period.endDate);
